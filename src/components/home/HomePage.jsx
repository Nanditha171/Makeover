    ...
    <div className="homepage-wrapper">
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          <div style={{ maxWidth: '720px' }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
              <button onClick={() => startBooking(null, 'salon')} className="btn btn-rose btn-lg" style={{ minWidth: '180px' }}>
            ...
            {/* Key Value Bar (Responsive 4-col on desktop / 2x2 grid on mobile) */}
      ...
      <section className="section-padding" style={{ background: 'var(--bg-secondary)' }}>
        <div className="container">
          ...
          <div className="grid-3">
            {whyChoose.map(item => (
              <div key={item.id} className="glass-card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            ...
            {services.slice(0, 6).map(srv => (
              <div key={srv.id} className="glass-card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '1.25rem', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    <h3 style={{ fontSize: '1.18rem', marginBottom: '0.35rem', color: 'var(--text-primary)' }}>{srv.name}</h3>
              ...
              <div key={pkg.id} className="glass-card" style={{
                ...
                <div>
                  ...
                  <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
                    <span style={{ fontSize: '1.9rem', fontWeight: '800', color: 'var(--primary-rose-dark)' }}>
                      {formatPrice(pkg.price)}
                    </span>
                    {pkg.originalPrice && (
                      <span style={{ textDecoration: 'line-through', color: 'var(--text-muted)', marginLeft: '0.5rem', fontSize: '0.95rem' }}>
        ...
        <div className="container">
          ...
          <div className="grid-4">
            {[
              ...
              <div key={item.step} className="glass-card" style={{ padding: '1.5rem', textAlign: 'center' }}>
                <div style={{ fontSize: '1.8rem', fontWeight: '800', color: 'var(--primary-rose-dark)', marginBottom: '0.4rem' }}>
                  {item.step}
                </div>
                <h3 style={{ fontSize: '1.1rem', color: 'var(--text-primary)', marginBottom: '0.35rem' }}>{item.title}</h3>
        ...
        <section className="section-padding" style={{ background: 'var(--bg-secondary)' }}>
          <div className="container">
            ...
            <div className="grid-2" style={{ maxWidth: '900px', margin: '0 auto' }}>
              {testimonials.map(t => (
                <div key={t.id} className="glass-card" style={{ padding: '1.4rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
        ...
        <div className="container" style={{ maxWidth: '800px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {faqs.map(faq => {
              ...
              return (
                <div key={faq.id} className="glass-card" style={{ padding: '1.1rem 1.25rem', borderColor: isOpen ? 'var(--primary-rose)' : 'var(--border-subtle)' }}>
                  <button
      ...
      <section style={{ background: 'var(--bg-secondary)', padding: '3.5rem 0', borderTop: '1px solid var(--border-subtle)' }}>
        <div className="container" style={{ textAlign: 'center', maxWidth: '750px' }}>
          <h3 style={{ fontSize: 'clamp(1.4rem, 3.5vw, 1.8rem)', marginBottom: '0.75rem', color: 'var(--text-primary)' }}>Visit Our Studio in Jubilee Hills</h3>
